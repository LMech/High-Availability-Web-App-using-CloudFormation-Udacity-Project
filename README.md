# High-Availability Web App using CloudFormation Udacity Project

This project is the final project for the IaC section in the Udacity Cloud DevOps NanoDegree which includes using CloudFormation to deploy a high-availability web app using aws services and cloudformation
...
- Each stack should be deployed independently ordered network, roles, then services to properly work.
- The stack can be create from the aws-cli using the `aws cloudformation create --region <region> --template-body file://<file-name>.yaml --parameters file://<file-name>.json`
- I the roles stack there is no need to add the `--parameters` as it's not needed and the `--capabilities "CAPABILITY_IAM" "CAPABILITY_NAMED_IAM"` flag should be added to be successfully deployed

## Notes

- The AMI value might change according the the deployment region
- This repository is for reference only and should not be used as solution for the same course